...
SessionStart injects one causal paragraph instead of the fragment pile: `recap [<age>]: <intent>: <completed>. Settled: <claim> (<receipt>). Now: <blockers|still-open|proposals|todos>. Learnt-then-retired while away: <label> (superseded <date>, <oid>). Next: <evidenced next step>.` Composer `src/hooks/recap.rs` + feeds `src/storage/recap_feeds.rs`, zero LLM. Every clause drops independently without evidence; `Next:` is never fabricated; receipts mandatory. Feeds fail open to the byte-identical fragment fallback. Suppressed from re-import via a machine-owned sentinel (`RECAP_SENTINEL`) checked in `provenance::is_csr_emission` ahead of quote-stripping and grammar-based detection — this narrows, not closes, new recap output re-entering the corpus (known bypasses: non-whitespace prefix on `recap [` defeats the header match, case-sensitive field tokens a real recap paragraph matches none of, and four call sites — `rerank.rs`, `session_briefing.rs`, `prompt_submit.rs`, `session_start.rs` — that skip quote-stripping before the check); it does not clean transcripts already embedded (747 conversations, known open debt). Kill switch `CSR_NO_RECAP=1`.
