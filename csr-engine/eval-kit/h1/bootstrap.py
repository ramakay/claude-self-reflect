#!/usr/bin/env python3
"""Paired bootstrap for the H1/H2 codegraph ablation (5 arms, see
examples/codegraph_ablation.rs and eval-kit/h1/score.py).
...
Metrics: origin-MRR (12 owner-mapped queries), nDCG@10 (20 graded queries). Per-query
paired deltas are resampled with replacement, 10,000 times, fixed seed 20260731; reports
mean delta and the 2.5/97.5 percentile bootstrap CI. Python stdlib only.

Run: python3 eval-kit/h1/bootstrap.py [path/to/results.json]
(results.json is produced by eval-kit/h1/score.py; defaults to eval-kit/h1/results.json)
"""
import json, os, random, sys

H1 = os.path.dirname(os.path.abspath(__file__))
results_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(H1, "results.json")
...
print(f"H1 (S_A > S AND S_A > S_Asham): {h1_verdict}")
print(f"H2 (S_F > S): {h2_verdict}")

summary["verdicts"] = {"h1": h1_verdict, "h2": h2_verdict}
out_path = os.path.join(H1, "bootstrap_results.json")
