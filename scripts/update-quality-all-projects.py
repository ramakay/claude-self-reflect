    ...
    for project_dir in claude_dir.iterdir():
        if project_dir.is_dir():
            ...
            if jsonl_files:
                ...
                # Try to extract the actual project name from the path
                # Format: -Users-username-projects-projectname
                parts = dir_name.split("-")
                if "projects" in parts:
                    idx = parts.index("projects")
                    if idx + 1 < len(parts):
                        project_name = "-".join(parts[idx + 1:])
                    else:
...
def update_quality_for_project(project_info):
    ...
    # Use conversation analyzer for all projects
    ...
    try:
        ...
        logger.info(f"✅ Quality updated: {result['quality_grade']} ({result['quality_score']:.1%})")
        logger.info(f"   Issues: {result['total_issues']}, Good patterns: {result['good_patterns_found']}")
        ...
        for cache_file in cache_files[:5]:  # Show first 5
            try:
                ...
                if data.get('status') == 'success':
                    ...
                    logger.info(f"   {cache_file.stem}: {summary['quality_grade']} ({summary['total_issues']} issues)")
            except:
                pass
