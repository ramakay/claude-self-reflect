...
Relocating rather than discarding the claim: what the AST layer is measured to deliver on this corpus is _integrity_. Every `calls` / `imports` edge terminates in an evidence-bearing state (witness closure 98.1% against a pre-registered bar of 90%); internal binding is 86.4% against a bar of 70%, on a denominator that excludes only edges with recorded proof of being external, method-dispatched, stale, or local; zero edges are drifted; and consumer surfaces label definition-backed versus name-only matches, carry line anchors, and abstain visibly rather than guess. Those are gate-measured properties, not MRR, and they are what makes a provenance answer trustworthy rather than merely ranked.
