...
#pad(left: 0.75in, right: 0.75in)[
  ...
  Across two within-operator corpora from unrelated codebases, reinstatement improved ground-truth session coverage over one-shot kNN by +53% and +47% at equal budget under pre-registered gates, with zero per-query losses in those coverage runs; blind cross-vendor judge panels preferred it on both corpora (instructions disclosed as hypothesis-aligned). On graded provenance gold derived from the operator's own ratification behavior (sealed pre-registered origin recall, dual-vendor dialog-act extraction, external ship-event ledgers), reinstatement wins on origin-MRR (+37%), `nDCG@10` (+29%), and graded recall (+25%) — point estimates on 12 mapped queries, without interval estimates — while both systems miss the verified origin conversation on over half the queries, locating the open headroom. On this decision-graded gold a channel ablation reverses the coverage-proxy attribution: code-graph spread, not semantic blending, carries the provenance signal. Evaluating the system surfaced a second finding: _self-indexing evaluation contamination_, in which the system ingests the evaluation dialogue itself and echoes of the questions displace the origin conversations being sought. A controlled dose-response experiment from a session-zero snapshot shows five scripted re-ask cycles capturing half of naive retrieval's top-10, with query-echo defenses repairing 72% of the echo occupancy (sham control null). A blinded three-arm behavioral benchmark (16 sealed re-litigation tasks) shows conversation memory roughly halving reason-quality failures on settled decisions relative to a no-memory arm — while reinstatement and one-shot kNN tie exactly in this single-hop regime, localizing reinstatement's advantage to multi-hop provenance; a pre-registered ratification-weighting hypothesis halted at $rho approx 0$ across three extractor generations, and that negative result directly forbade an inference-based staleness design in the deployed system, yielding an asserted-state resolution ledger instead. We argue that frozen pre-evaluation snapshots, shared index builds, and echo defenses are necessary controls for any self-recording agent memory. External and multi-operator validation remain open.
]
...
Four findings. (1) _Graph spread is the workhorse_: graph-only beats the full walk on every metric despite the scoring bias against it — code-graph structure, not semantic blending, carries the provenance signal on graded gold. This reverses the spike-era attribution, and we treat the graded result as authoritative: blend dominance was an artifact of file-touch _coverage_ gold; on decision-graded gold — which derives from dialog-acts and ship ledgers, independent of the `code_evolution` plumbing the graph channel shares with the coverage GT — graph dominance is not self-confirming. (2) _Fusion dilutes_: max-score fusion lets blend-sourced semantic neighbors crowd graph-sourced provenance neighbors out of the final budget; the tuning direction is a larger graph share, not more channels. (3) _Echo defense is causally necessary_: removing it drops origin-MRR to 0.234 — below the kNN baseline. (4) _The episode chain, previously a null result, is a strong origin-finder on graded gold_ (0.321 origin-MRR) though weak on evidence depth — it finds the origin thread, not the full evidence set.
