...
The E2 gold construction suggested a natural extension: if ratification behavior can grade relevance, perhaps it can weight retention. We tested whether a _global_ per-conversation ratification score — DIRECTS / ACCEPTS / REJECTS dialog-acts extracted by an LLM over each conversation, corroborated against the external git ledger, with uncorroborated scores capped at 0.6 — ranks decision origins. The pre-registered gate was Spearman correlation between backfilled scores and the sealed E2 grades (conversation-level, taking the max grade across queries), with an explicit halt condition at $rho approx 0$. The gate failed twice. Run 1 measured $rho = 0.060$ but was an extraction artifact: 88% of conversations yielded zero acts (the digest sampled mostly assistant and tool text, and the echo-hardened prompt bailed to empty output on instruction-like digests — which all agent digests are). Run 2, under a v2 operator-turn-prioritized extractor, was clean — scores had variance and acts were genuinely extracted — and still measured $rho = 0.071$ ($n = 123$); grade-3 sealed origins averaged a score of 0.362 against 0.422 for grade-0 incidental conversations.
...
*Ruled out by pre-registration:* global per-conversation ratification weighting as a retention signal — Spearman against the sealed graded gold measured $rho = 0.060 arrow.r 0.071$ across two extractor versions ($n = 123$), halting that program at its pre-registered gate; only the edge-level, query-conditional formulation remains open.
