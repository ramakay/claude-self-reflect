...
**Staleness baseline (T2, before-arm measured).** Replaying the 42 verified-resolved task topics as retrieval queries against the live index: **47.4% of top-5 results (99/209) presented the resolved item as still open**; 41/42 queries surfaced at least one stale result; several rank the stale claim above the resolution evidence. This quantifies, at n = 42, the staleness cost previously observed anecdotally, and is the pre-write baseline for the ledger-demotion replay.
