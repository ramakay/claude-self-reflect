...
Across two within-operator corpora from unrelated codebases, reinstatement improved ground-truth session coverage over one-shot kNN by +53% and +47% at equal budget under pre-registered gates, with zero per-query losses; blind cross-vendor judge panels preferred it on both corpora. Evaluating the system surfaced a second finding: *self-indexing evaluation contamination*, in which the system ingests the evaluation dialogue itself and echoes of the questions displace the origin conversations being sought. Query-echo defenses repaired the measured displacement while holding coverage. We argue that frozen pre-evaluation snapshots, shared index builds, and echo defenses are necessary controls for any self-recording agent memory. External and multi-operator validation remain open.
...
2. **Deliberation** — the observable assistant and tool trajectory: emitted reasoning, tool-call traces, dead ends, and sidechain/subagent transcripts (what the harness records, not the model’s internal computation).
...
## Addendum (2026-07-27, draft): Ratification at the Point of Exchange

*Results in this section are measured but preliminary; the demotion replay (T2) and the cross-source three-arm benchmark (T3) are pre-registered and in progress. Numbers below will be finalized in the next revision.*

The ratification-extraction failure reported above (ρ ≈ 0.04 across three extractors; settlement cannot be reliably inferred from dialogue prose) prompted a reframing that this addendum documents: **ratification does not need to be reconstructed after the fact, because for most engineering work it is recorded at the moment of exchange by systems external to both parties.** When the agent runs `git commit` or `git push`, the CLI's reply — echoed into the session transcript as a tool result — is a machine-issued receipt that the artifact landed. The transcript corpus, it turns out, is full of these.

**Receipt taxonomy (measured).** Of 529 commits across four repositories (June–July 2026), 509 (96%) have their hash present in indexed conversation chunks. Classifying by JSON context in the raw transcripts: **423 (80%) are receipt-class** — the hash appears inside a tool result of the git invocation itself, identifying the authoring session by exact key — and 86 are citation-class (prose mentions, typically later sessions discussing the commit). Receipts convert the deliberation→artifact join from fuzzy semantic correlation into an exact-key join; they are the only edge class in the system that is collision-free by construction.

**Task-lifecycle signal (T1, measured).** Mining the same transcripts for task-management tool calls yielded 219 task creations and 204 completed lifecycles over two months. A blind audit of 50 sampled completions against git ground truth (adversarial protocol, default-refute, commit-hash evidence per row) measured **precision 0.913** on gradeable rows (42y/4n/4u; 0.840 counting ungradeable as failures; Wilson 95% CI ≈ 0.79–0.97 at this n). The failure texture is typed rather than random: all four false completions came from sessions with **zero receipts in the task window** (three from a single planning session whose tasks were marked complete with no commits anywhere that day; one bug marked fixed whose handler was last modified two months prior). Every receipted completion graded true. A look-ahead scan found zero revert commits across all four repositories in the measurement window, closing the exposure period on the sample.

**Design consequence.** The resolution ledger's promotion gate becomes *receipt presence*, not retrospective human ratification: task completed + matching receipt in the session window → auto-resolve; receipt-less completions remain advisory. Structural corroboration via AST anchors — originally proposed as the primary confirmation mechanism — demotes to a fallback for artifact-free claims. The failed extraction line and this result compose into a single ladder: settlement inferred from prose is unreliable (ρ ≈ 0.04); settlement asserted by lifecycle metadata is 0.91-trustworthy; settlement confirmed by exchange-time receipts was, on this sample, always true.

**Staleness baseline and demotion replay (T2, measured).** Replaying the 42 verified-resolved task topics as retrieval queries against the live index: **47.4% of top-5 results (99/209) presented the resolved item as still open**; 41/42 queries surfaced at least one stale result; several ranked the stale claim above the resolution evidence. Ledger verdicts were then written for the 32 stale instances passing a strict chunk-identity filter (57 candidates skipped as ambiguous — a wrong resolve on a still-open item being the worst outcome, precision was preferred over coverage). On replay, **write-target staleness fell from 100% to 0%**: 25 instances remained visible but annotated and demoted to the bottom of the page regardless of raw similarity score (several were the top-scored result in their set), 7 left the top-5 entirely, and no case was observed of demotion suppressing fresh, still-relevant content. A regression round-trip on one verdict (resolved → regressed → resolved) resurfaced the chunk at rank 2 un-demoted and re-demoted it to rank 5 on flip-back, confirming the append-only ledger's reversibility in production.

**Relation to the main results.** The receipts reframing does not alter the retrieval claims above; it supplies the settlement layer those claims lacked. The `supersedes` population problem ("never populated by any live pipeline") and the multi-hop benchmark's need for cross-source ground truth both reduce, in large part, to reading receipts that were already retained.

**T3 outcome (pre-registered, both gates resolved; full protocol and numbers in `saga-t3-results.md`).** The cross-source benchmark this addendum built toward has run, and the honest result reverses this paper's central retrieval claim on its own corpus: **Gate M failed decisively** (receipt lookup: kNN+FTS 0.783 vs reinstatement 0.545 recall@5, sign test p ≈ 0, n = 396 mechanically-labeled pairs) and **Gate D was null** (27 sealed questions, cross-family blinded judges: multi-hop subset R-only 0 vs K-only 2 discordant, p = 0.50). Per the pre-committed failure interpretation, the reinstatement walk's advantage over flat retrieval is not supported here — replicating, on an independent git-grounded question set with external judges, the earlier 16-task tie. What the benchmark instead established: memory versus no-memory is not close (no-memory scored 0/27 consensus-correct; every correct answer depended on retrieved context), and exact-key structural evidence — the receipts this addendum documents — is the single strongest retrieval signal measured, which is precisely why the walk lost to an arm that could read it directly. The claim that survives is narrower and better-grounded than the one we set out to defend: *what compounds memory's value is not more elaborate recall, but settlement — receipts, verdicts, and demotion keeping the recalled past true.*
