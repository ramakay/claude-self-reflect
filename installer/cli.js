...
import { findEngineBinary, whichBinary } from './lib.js';
 ...
 */
function findBinary() {
  const installDir = resolve(process.env.CSR_INSTALL_DIR || join(homedir(), '.local', 'bin'));
  return findEngineBinary({ installDir, pathBinary: whichBinary() });
}
